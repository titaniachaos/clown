...
| `check-images.mjs` | an image over the weight budget, pixel dimensions far past any slot on the page, a picture that costs too many bytes per pixel, and — the one no diff shows — `seo.ts` declaring dimensions the file does not have, which is what a social platform reads before it fetches anything. An unreferenced image fails: a file genuinely linked from outside the repository is named in `KEPT` with the reason, so the exception is written down rather than assumed |
| `check-receipt.mjs` | the citations feed drifting from the bibliography it publishes. The feed is the one artefact that leaves the site, so it carries an order-invariant SHA-256 receipt over the works, and the check recomputes it twice: once from the bibliography module, once from the built feed alone, parsed back out as a stranger would. Route two is what earns it — a work dropped by a filter or an author list mangled by escaping moves the feed's fold without touching the ledger. It also catches a feed edited after the build, and a fold that stops being order-invariant |
